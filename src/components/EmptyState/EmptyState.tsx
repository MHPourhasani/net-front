...
const EmptyState = ({ imgSrc, className, imgClassName, title, description, linkHref, linkTitle }: Props) => {
    return (
        <div className={`flex h-full w-full flex-1 flex-col items-center justify-center gap-4 p-4 md:p-0 ${className}`}>
            ...
            {linkHref && linkTitle ? (
                <Link
                    to={linkHref}
                    className="flex w-9/12 items-center justify-center rounded-xl bg-sky-400 py-2.5 text-white transition-all ease-in-out hover:bg-sky-500 md:max-w-3xl lg:py-3 xl:max-w-[400px]"
                >
                    {linkTitle}
                </Link>
            ) : null}
